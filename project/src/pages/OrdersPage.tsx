...
// Extended shop profile — used in invoice rendering only
type ShopInfo = {
  shop_name:               string;
  phone?:                  string | null;
...
function buildPrintHTML(order: Order, items: OrderItem[]): string {
  ...
  // ── MIHWAR Verification Center wordmark ──
  const logoSVG = `<div style="display:flex;flex-direction:column;gap:5px;align-items:flex-start;">
    ...
    <div style="font-family:'IBM Plex Sans Arabic',Tahoma,Arial;font-weight:400;font-size:9px;color:#10B981;letter-spacing:0.2px;">✓ تم التحقق من صحة المستند بنجاح</div>
  ...
  // ── Render non-null shop fields as <div class="icr"> rows ──
  function shopInfoRows(
    shop: ShopInfo | undefined,
    fields: { key: keyof ShopInfo; label: string }[]
  ): string {
    if (!shop) return "";
    return fields
      .filter(({ key }) => {
      ...
      .map(({ key, label }) => `
        <div class="icr">
          ...
          <span class="icv">${escapeHTML(String(shop[key]))}</span>
        </div>`)
      .join("");
  }

  // ── Shop logo in card header — improved rendering ──
  function shopLogoImg(shop: ShopInfo | undefined): string {
    if (!shop?.logo_url) return "";
    return `<div style="width:60px;height:60px;border-radius:10px;background:#fff;
             box-shadow:0 1px 6px rgba(0,0,0,0.12);overflow:hidden;
             flex-shrink:0;margin-left:10px;display:flex;align-items:center;justify-content:center;">
      <img src="${escapeHTML(shop.logo_url)}" alt="logo"
...
.title-ar { font-size: 26px; font-weight: 800; color: var(--navy); font-family: 'IBM Plex Sans Arabic', Tahoma; line-height: 1; letter-spacing: 0.02em; }
.title-en { font-size: 11px; font-weight: 600; color: var(--muted); letter-spacing: 3px; text-transform: uppercase; margin-top: 4px; display: block; }
...
.sbadge { display: inline-flex; align-items: center; gap: 5px; padding: 4px 12px 4px 10px; border-radius: 20px; border: none; font-size: 11px; font-weight: 700; font-family: Tahoma; }
...
.ich-left { display: flex; align-items: center; gap: 0; flex: 1; min-width: 0; }
...
.ick { font-size: 10px; color: var(--muted); font-weight: 600; font-family: Tahoma; white-space: nowrap; flex-shrink: 0; }
...
table.items thead th { padding: 10px 12px; font-size: 9.5px; font-weight: 700; letter-spacing: 0.8px; text-transform: uppercase; color: #93C5FD; text-align: right; white-space: nowrap; border-bottom: 1px solid #1E3A8A; }
...
.stot { background: var(--blue1); padding: 14px 14px; display: flex; justify-content: space-between; align-items: center; }
...
@media print {
  ...
  /* Trust + authenticity blocks */
  .trust-block, .auth-block { page-break-inside: avoid; }
}
</style>
</head>
<body>
...
<div class="icard-grid">
  ...
  <div class="icard">
    <div class="ich">
      <div class="ich-left">
        ${shopLogoImg(order.from_shop)}
        <div>
          <div class="ich-ar">بيانات المشتري</div>
          <div class="ich-en">Buyer Information</div>
        </div>
      </div>
  ...
  <div class="icard">
    <div class="ich">
      <div class="ich-left">
        ${shopLogoImg(order.to_shop)}
        <div>
          <div class="ich-ar">بيانات المورد</div>
          <div class="ich-en">Supplier Information</div>
        </div>
      </div>
      <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="#93C5FD" stroke-width="2">
...
<div class="qsrow">
  <div class="qrcard">
    <div class="qrch">
      ...
      <span class="qrcen">MIHWAR Verification Center</span>
    </div>
    <div class="qrcb">
      ...
      <div style="text-align:center;margin-top:6px;">
        ...
        <span class="qrhint" style="display:block;margin-top:3px;font-family:Tahoma,Arial;font-size:8px;color:#64748B;line-height:1.4;">امسح للتحقق من صحة المستند</span>
        <span style="display:block;margin-top:2px;font-family:Tahoma,Arial;font-size:7.5px;color:#10B981;font-weight:600;">✓ Verified by MIHWAR</span>
        <!-- 3. Verification URL — visible for PDF copy -->
        <span style="display:block;margin-top:5px;font-family:'JetBrains Mono',monospace;font-size:6.5px;color:#94A3B8;word-break:break-all;text-align:center;line-height:1.5;border-top:1px solid #E2E8F0;padding-top:4px;">${verifyUrl}</span>
      </div>
    </div>
  </div>
...
<!-- 2. VERIFIED BY MIHWAR TRUST BLOCK -->
<div style="margin-top:10px;border:1.5px solid #BBF7D0;border-radius:9px;overflow:hidden;page-break-inside:avoid;">
  <div style="background:#DCFCE7;padding:7px 14px;display:flex;align-items:center;gap:8px;">
    <span style="font-size:13px;">✓</span>
    <div>
      <div style="font-family:'IBM Plex Sans Arabic',Tahoma;font-weight:700;font-size:11px;color:#166534;">Verified by MIHWAR · موثق عبر منصة محور</div>
    </div>
  </div>
  <div style="background:#F0FDF4;padding:8px 14px;display:flex;justify-content:space-between;align-items:center;gap:12px;">
    <div style="font-family:'IBM Plex Sans Arabic',Tahoma;font-size:9.5px;color:#15803D;line-height:1.6;">
      تم إصدار هذا المستند والتحقق من صحته إلكترونياً عبر منصة محور.
    </div>
    <div style="font-family:Tahoma,Arial;font-size:9px;color:#16A34A;white-space:nowrap;font-style:italic;">
      This document has been digitally verified through MIHWAR Marketplace.
    </div>
  </div>
</div>

<!-- 7. DOCUMENT AUTHENTICITY SECTION -->
<div style="margin-top:10px;border:1.5px solid var(--border);border-radius:9px;overflow:hidden;page-break-inside:avoid;">
  <div style="background:var(--navy);padding:6px 14px;display:flex;align-items:center;justify-content:space-between;">
    <span style="font-family:'IBM Plex Sans Arabic',Tahoma;font-weight:700;font-size:10px;color:#fff;letter-spacing:0.3px;">Document Authenticity · موثق إلكترونياً عبر منصة محور</span>
  </div>
  <div style="background:var(--bg);padding:8px 14px;display:grid;grid-template-columns:1fr 1fr 1fr;gap:4px 16px;">
    <div style="font-size:9px;color:var(--muted);">
      <span style="font-weight:700;color:var(--muted2);display:block;font-size:8px;text-transform:uppercase;letter-spacing:0.5px;">Document ID</span>
      <span style="font-family:'JetBrains Mono',monospace;color:var(--navy);font-weight:700;">${poNumber}</span>
    </div>
    <div style="font-size:9px;color:var(--muted);">
      <span style="font-weight:700;color:var(--muted2);display:block;font-size:8px;text-transform:uppercase;letter-spacing:0.5px;">Verification Status</span>
      <span style="color:#16A34A;font-weight:700;">✓ Verified</span>
    </div>
    <div style="font-size:9px;color:var(--muted);">
      <span style="font-weight:700;color:var(--muted2);display:block;font-size:8px;text-transform:uppercase;letter-spacing:0.5px;">Verification Time</span>
      <span style="font-family:'JetBrains Mono',monospace;color:var(--navy);font-weight:600;">${printDate} ${printTime}</span>
    </div>
  </div>
</div>
...
<div class="ftr" style="text-align:center;justify-content:center;flex-direction:column;gap:5px;padding:12px 22px;">
  <div style="font-family:'IBM Plex Sans Arabic',Tahoma;font-weight:800;font-size:16px;color:#fff;letter-spacing:0.5px;">محور · MIHWAR</div>
  <div style="font-family:Tahoma,Arial;font-weight:600;font-size:9px;color:#93C5FD;letter-spacing:2px;text-transform:uppercase;">MIHWAR Verification Center</div>
  <div style="font-family:Tahoma,Arial;font-size:8.5px;color:#60A5FA;">Powered by MIHWAR Marketplace · منصة محور لقطع غيار المركبات</div>
  <div style="font-family:'JetBrains Mono',monospace;font-size:8px;color:#BFDBFE;margin-top:2px;">www.mihwarb2b.com &nbsp;·&nbsp; ${poNumber} &nbsp;·&nbsp; ${printDate}</div>
</div>
...
// ─────────────────────────────────────────────────────────────
// MAIN COMPONENT — unchanged from original except fetchOrders query
// ─────────────────────────────────────────────────────────────
...
export default function OrdersPage() {
  ...
  // Extended query — fetches full shop profile for invoice rendering
  ...
  const { filtered, counts } = useMemo(() => {
    ...
    return {
      ...
      counts: {
        ...
        rejected: rejectedCount, completed: completedCount, totalValue,
      },
    };
    ...
    <div className="p-4 lg:p-8 min-h-screen pb-24 md:pb-10" dir={isRTL ? "rtl" : "ltr"}>
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
        ...
        <div className="flex items-center gap-2">
          ...
          {!isAdmin && (
            <button onClick={() => setShowModal(true)} className="flex items-center justify-center gap-2 bg-blue-600 hover:bg-blue-500 text-white px-6 py-3 rounded-xl font-bold shadow-lg shadow-blue-900/20 active:scale-95 transition-all">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 mb-6">
        {[
          ...
          { label: t("Approved", "مقبولة"),           val: counts.approved + counts.completed,           icon: CheckCircle,  color: "text-blue-400",   bg: "bg-blue-500/5"   },
      ...
      <div className="flex flex-col lg:flex-row gap-3 mb-5">
        <div className="flex bg-slate-900 p-1 rounded-xl border border-slate-800 overflow-x-auto no-scrollbar shrink-0" role="tablist">
          {(["all", "incoming", "outgoing"] as const).map(k => (
            <button key={k} role="tab" aria-selected={tab === k} onClick={() => handleTabChange(k)}
            ...
            {pageItems.length === 0 ? (
              ...
              <tr key={o.id} className="hover:bg-slate-800/20 transition-colors">
                ...
                <td className={`p-4 font-bold ${o.from_shop_id === ownedShopId ? "text-blue-400" : "text-white"}`}>
                  ...
                  {o.from_shop_id === ownedShopId && <span className="text-[9px] opacity-50 font-medium px-1 bg-blue-500/10 rounded ml-1">أنت</span>}
                </td>
                <td className={`p-4 font-bold ${o.to_shop_id === ownedShopId ? "text-emerald-400" : "text-white"}`}>
                  {o.to_shop?.shop_name}
                  {o.to_shop_id === ownedShopId && <span className="text-[9px] opacity-50 font-medium px-1 bg-emerald-500/10 rounded ml-1">أنت</span>}
                </td>
                ...
                <td className="p-4 text-center">
                  <button onClick={() => openDetail(o)} className="p-2 text-slate-400 hover:text-white hover:bg-slate-800 rounded-lg transition-all" aria-label={t("View order details", "عرض تفاصيل الطلب")}>
                    <Eye size={18} />
                  </button>
                </td>
              </tr>
          ...
          <div className="relative bg-slate-900 border border-slate-800 w-full max-w-2xl lg:rounded-3xl rounded-t-[2rem] shadow-2xl flex flex-col max-h-[94vh] overflow-hidden">
            <div className="flex items-center justify-between px-5 py-4 border-b border-slate-800 shrink-0 bg-slate-900/80 backdrop-blur-md">
              <div className="min-w-0 flex-1">
                <div className="flex items-center gap-3 flex-wrap">
                  <h2 className="text-lg font-black text-white">{t("Order", "طلب")} <span className="font-mono text-slate-400">#{detailOrder.id}</span></h2>
              ...
              <div className="flex items-center gap-2 shrink-0 ml-3">
                <button onClick={handlePrint} className="flex items-center gap-1.5 px-3 py-2 rounded-xl bg-slate-800 text-slate-300 hover:text-white text-xs font-bold transition-all active:scale-95" aria-label={t("Print order", "طباعة الطلب")}>
                  <Printer size={15} />
                  <span className="hidden sm:inline">{t("Print", "طباعة")}</span>
                </button>
                <button onClick={() => setDetailOrder(null)} className="p-2 rounded-xl bg-slate-800 text-slate-400 hover:text-white transition-all active:scale-95" aria-label={t("Close", "إغلاق")}><X size={18} /></button>
              </div>
            </div>
                ...
                {detailLoading ? (
                  ...
                  <div className="space-y-2">
                    {detailItems.map(item => (
                      <div key={item.id} className="bg-slate-800/30 border border-slate-800/50 px-4 py-3 rounded-xl flex justify-between items-center hover:border-slate-700 transition-colors">
                        <div className="flex-1 min-w-0">
            ...
            <div className="px-5 py-4 border-t border-slate-800 bg-slate-900 shrink-0 pb-[calc(5.5rem+env(safe-area-inset-bottom))] lg:pb-[calc(1rem+env(safe-area-inset-bottom))]">
              <div className="flex items-center justify-between mb-4">
                ...
                <span className="text-2xl font-black text-emerald-400">{detailOrder.total_amount.toLocaleString()}<span className="text-sm font-normal text-slate-400"> ر.س</span></span>
              </div>
              {canActOnOrder(detailOrder) && (
                <div className="flex gap-3">
                  <button onClick={() => handleReject(detailOrder.id)} disabled={actionId === detailOrder.id} className="flex-1 py-3.5 rounded-2xl border border-red-500/20 text-red-400 font-bold hover:bg-red-500/10 transition-all flex items-center justify-center gap-2 active:scale-95 disabled:opacity-50">
                    <XCircle size={18} /> {t("Reject", "رفض")}
                  </button>
                  <button onClick={() => handleApprove(detailOrder.id)} disabled={actionId === detailOrder.id} className="flex-[2] py-3.5 rounded-2xl bg-emerald-600 hover:bg-emerald-500 text-white font-black shadow-lg shadow-emerald-900/20 transition-all flex items-center justify-center gap-2 active:scale-95 disabled:opacity-50">
                    {actionId === detailOrder.id ? <RefreshCw className="animate-spin" size={18} /> : <PackageCheck size={18} />}
        ...
        <div className="fixed inset-0 z-[110] flex items-end lg:items-center justify-center lg:p-4 animate-in fade-in duration-200" role="dialog" aria-modal="true" aria-label={t("Create Purchase Order", "إنشاء طلب شراء")}>
          ...
          <div className="relative bg-slate-900 border border-slate-800 w-full max-w-3xl lg:rounded-3xl rounded-t-[2rem] shadow-2xl flex flex-col max-h-[96vh] overflow-hidden">
            <div className="flex items-center justify-between px-6 py-4 border-b border-slate-800 shrink-0">
              <h2 className="text-lg font-black text-white flex items-center gap-2"><Plus className="text-blue-400" size={20} />{t("Create Purchase Order", "إنشاء طلب شراء جديد")}</h2>
              <button onClick={() => setShowModal(false)} className="p-2 text-slate-500 hover:text-white active:scale-90 transition-all" aria-label={t("Close", "إغلاق")}><X size={22} /></button>
            </div>
            <div className="p-5 overflow-y-auto no-scrollbar space-y-5 flex-1">
              ...
              <div className="space-y-2">
                ...
                <select value={supplierShopId} onChange={e => { setSupplierShopId(Number(e.target.value) || ""); setCart([]); }} className="w-full bg-slate-800 border border-slate-700 rounded-xl p-3.5 text-white focus:border-blue-500 outline-none transition-all cursor-pointer text-sm">
              ...
              {supplierShopId && (
                <div className="space-y-3">
                  <div className="flex items-center justify-between gap-3">
                    <h3 className="text-[10px] font-black text-slate-500 uppercase tracking-widest shrink-0">{t("Available Catalog", "المنتجات المتوفرة")}</h3>
                    <div className="relative flex-1 max-w-[180px]">
                      ...
                      <input value={productSearch} onChange={e => setProductSearch(e.target.value)} placeholder={t("Search...", "بحث...")} className={`w-full bg-slate-950 border border-slate-800 rounded-lg py-2 ${isRTL ? "pr-8 pl-3" : "pl-8 pr-3"} text-xs text-white outline-none focus:border-blue-500`} />
                    </div>
                  </div>
                  {loadingProducts ? (
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 max-h-52 overflow-y-auto no-scrollbar">
                      {filteredSupplierProducts.map(p => (
                        <div key={p.id} className="bg-slate-800/40 border border-slate-800 p-3 rounded-xl flex justify-between items-center hover:border-blue-500/30 transition-all">
                          <div className="min-w-0 flex-1">
                          ...
                          <div className="flex items-center gap-2 shrink-0 ml-2">
                            ...
                            <button onClick={() => addToCart(p)} className="p-1.5 bg-blue-600 text-white rounded-lg hover:bg-blue-500 active:scale-90 transition-all" aria-label={t(`Add ${p.part_name}`, `إضافة ${p.part_name}`)}><Plus size={14} /></button>
                          </div>
                        </div>
                      ))}
                    </div>
                      ...
                      <div key={c.product.id} className="bg-slate-950/40 px-3 py-2.5 rounded-xl flex items-center justify-between border border-slate-800/50">
                        ...
                        <div className="flex items-center gap-2 shrink-0">
                          <div className="flex items-center bg-slate-800 rounded-lg p-0.5 border border-slate-700">
                            <button onClick={() => updateQty(c.product.id, c.quantity - 1)} className="w-7 h-7 flex items-center justify-center text-slate-400 hover:text-white text-sm font-bold transition-colors rounded-md active:bg-slate-700">−</button>
                            <span className="w-7 text-center font-black text-white text-sm">{c.quantity}</span>
                            <button onClick={() => updateQty(c.product.id, c.quantity + 1)} className="w-7 h-7 flex items-center justify-center text-slate-400 hover:text-white text-sm font-bold transition-colors rounded-md active:bg-slate-700">+</button>
                          </div>
                          <button onClick={() => removeFromCart(c.product.id)} className="text-slate-700 hover:text-red-400 p-1.5 active:scale-90 transition-all" aria-label={t("Remove", "إزالة")}><Trash2 size={15} /></button>
                        </div>
                      </div>
                    ))}
                  </div>
              ...
              <div className="space-y-2">
                ...
                <textarea value={orderNotes} onChange={e => setOrderNotes(e.target.value)} placeholder={t("Any special delivery or order instructions...", "تعليمات خاصة بالتوصيل أو التجهيز...")} className="w-full bg-slate-800 border border-slate-700 rounded-xl p-3.5 text-white outline-none focus:border-blue-500 min-h-[80px] text-sm leading-relaxed transition-colors resize-none" />
              </div>
            </div>
            <div className="px-5 py-4 border-t border-slate-800 bg-slate-900 shrink-0 pb-[calc(5.5rem+env(safe-area-inset-bottom))] lg:pb-[calc(1rem+env(safe-area-inset-bottom))]">
              <button onClick={handleSubmit} disabled={saving || cart.length === 0} className="w-full py-4 bg-blue-600 hover:bg-blue-500 text-white font-black rounded-2xl shadow-xl shadow-blue-900/30 active:scale-[0.98] transition-all flex items-center justify-center gap-3 disabled:opacity-50 text-sm">
