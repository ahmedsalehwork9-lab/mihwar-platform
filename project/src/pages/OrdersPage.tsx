import { useEffect, useState, useMemo } from "react";import { supabase } from "../lib/supabase";import { useAuth } from "../context/AuthContext";import {ShoppingCart, RefreshCw, Package, Plus, X,Check, XCircle, Clock, PackageCheck,Eye, Trash2, AlertCircle, ChevronLeft, ChevronRight,ArrowLeftRight, Search, Save, ChevronDown,Printer, FileText, Download, Share2, Phone, MessageCircle, Calendar, Store, User, CheckCircle2} from "lucide-react";import QRCode from "react-qr-code";

// ─────────────────────────────────────────────────────────────// TYPES// ─────────────────────────────────────────────────────────────

type OrderStatus = "pending" | "approved" | "rejected" | "completed";

type Order = {id: number;from_shop_id: number;to_shop_id: number;status: OrderStatus;total_amount: number;notes: string | null;created_at: string;from_shop?: { shop_name: string; city?: string; phone?: string };to_shop?:   { shop_name: string; city?: string; phone?: string };order_items?: { id: number }[];};

type OrderItem = {id: number;order_id: number;product_id: number;quantity: number;price: number;product?: Product;};

type Shop = {id: number;shop_name: string;phone?: string;city?: string;};

type Product = {id: number;part_name: string;part_number: string;brand: string;model: string;quantity: number;price: number;shop_id: number;};
...
const STATUS_META: Record<OrderStatus, { label: string; color: string; bg: string; dot: string; icon: any }> = {pending:   { label: "معلق",   color: "text-amber-400", bg: "bg-amber-500/10", dot: "bg-amber-400", icon: Clock },approved:  { label: "مقبول",  color: "text-blue-400",  bg: "bg-blue-500/10",  dot: "bg-blue-400",  icon: CheckCircle2 },rejected:  { label: "مرفوض", color: "text-red-400",   bg: "bg-red-500/10",   dot: "bg-red-400",   icon: XCircle },completed: { label: "مكتمل", color: "text-emerald-400", bg: "bg-emerald-500/10", dot: "bg-emerald-400", icon: PackageCheck },};

// ─────────────────────────────────────────────────────────────// PRINT / PDF HELPER (PROFESSIONAL INVOICE)// ─────────────────────────────────────────────────────────────

function buildPrintHTML(order: Order, items: OrderItem[]): string {const date = new Date(order.created_at).toLocaleString("ar-SA", {day: "numeric", month: "long", year: "numeric",});const vat = Number(order.total_amount) * 0.15;const subtotal = Number(order.total_amount) - vat;
...
export default function OrdersPage() {const { ownedShopId, isAdmin } = useAuth() as any;

/* ── data ─────────────────────────────────── */const [orders,   setOrders]   = useState<Order[]>([]);const [shops,    setShops]    = useState<Shop[]>([]);const [loading,  setLoading]  = useState(false);const [error,    setError]    = useState<string | null>(null);const [toast,    setToast]    = useState<string | null>(null);

/* ── table ────────────────────────────────── */const [tab,          setTab]          = useState<"all"|"incoming"|"outgoing">("all");const [statusFilter, setStatusFilter] = useState<"all"|OrderStatus>("all");const [search,       setSearch]       = useState("");const [page,         setPage]         = useState(1);

/* ── detail drawer ────────────────────────── */const [detailOrderId,  setDetailOrderId]  = useState<number | null>(null);const [detailItems,    setDetailItems]    = useState<OrderItem[]>([]);const [detailLoading,  setDetailLoading]  = useState(false);

/* ── new order modal ──────────────────────── */const [showModal,        setShowModal]        = useState(false);const [supplierShopId,   setSupplierShopId]   = useState<number | "">("");const [supplierProducts, setSupplierProducts] = useState<Product[]>([]);const [loadingProducts,  setLoadingProducts]  = useState(false);const [productSearch,    setProductSearch]    = useState("");const [cart,             setCart]             = useState<CartItem[]>([]);const [orderNotes,       setOrderNotes]       = useState("");const [modalError,       setModalError]       = useState<string | null>(null);const [saving,           setSaving]           = useState(false);

/* ── action loading ───────────────────────── */const [actionId, setActionId] = useState<number | null>(null);

const detailOrder = useMemo(() => orders.find(o => o.id === detailOrderId) ?? null,[orders, detailOrderId]);

const fetchOrders = async (): Promise<Order[]> => {if (!isAdmin && !ownedShopId) return [];setLoading(true);try {let query = supabase.from("orders").select(          *,
          from_shop:shops!orders_from_shop_id_fkey(shop_name, city, phone),
          to_shop:shops!orders_to_shop_id_fkey(shop_name, city, phone),
          order_items(id)
       );if (!isAdmin) {query = query.or(from_shop_id.eq.${ownedShopId},to_shop_id.eq.${ownedShopId});}const { data, error: fetchError } = await query.order("created_at", { ascending: false });if (fetchError) throw fetchError;const fetched = (data as Order[]) || [];setOrders(fetched);return fetched;} catch (e: any) {setError(e.message);return [];} finally {setLoading(false);}};

const fetchShops = async () => {const { data } = await supabase.from("shops").select("id, shop_name, phone, city").order("shop_name");setShops((data as Shop[]) || []);};

useEffect(() => {fetchOrders();fetchShops();}, [ownedShopId, isAdmin]);

useEffect(() => {if (!supplierShopId) { setSupplierProducts([]); return; }setLoadingProducts(true);supabase.from("products").select("*").eq("shop_id", supplierShopId).gt("quantity", 0).order("part_name").then(({ data }) => { setSupplierProducts((data as Product[]) || []); setLoadingProducts(false); });}, [supplierShopId]);

const filtered = useMemo(() => {const q = search.trim().toLowerCase();return orders.filter(o => {if (isAdmin) return true;if (tab === "incoming") return o.to_shop_id   === ownedShopId;if (tab === "outgoing") return o.from_shop_id === ownedShopId;return true;}).filter(o => statusFilter === "all" || o.status === statusFilter).filter(o => !q || String(o.id).includes(q) || o.from_shop?.shop_name?.toLowerCase().includes(q) || o.to_shop?.shop_name?.toLowerCase().includes(q));}, [orders, tab, statusFilter, search, ownedShopId, isAdmin]);
...
const counts = useMemo(() => ({all:      orders.length,incoming: isAdmin ? orders.length : orders.filter(o => o.to_shop_id   === ownedShopId).length,outgoing: isAdmin ? orders.length : orders.filter(o => o.from_shop_id === ownedShopId).length,pending:  orders.filter(o => o.status === "pending").length,}), [orders, ownedShopId, isAdmin]);

const openDetail = async (order: Order) => {setDetailOrderId(order.id);setDetailLoading(true);const { data } = await supabase.from("order_items").select(", product()").eq("order_id", order.id);setDetailItems((data as OrderItem[]) || []);setDetailLoading(false);};

const handlePrint = () => {if (!detailOrder) return;const html = buildPrintHTML(detailOrder, detailItems);const win = window.open("", "_blank");if (!win) return;win.document.write(html);win.document.close();win.onload = () => { win.focus(); win.print(); };};

const handleApprove = async (orderId: number) => {if (actionId !== null) return;setActionId(orderId);try {const { error: rpcError } = await supabase.rpc("approve_order", { p_order_id: orderId });if (rpcError) throw rpcError;await fetchOrders();setToast("تم اعتماد الطلب وتحديث المخزون ✓");} catch (e: any) {setError(e.message);} finally {setActionId(null);}};

const handleReject = async (orderId: number) => {if (!confirm("رفض الطلب؟")) return;if (actionId !== null) return;setActionId(orderId);try {const { error: rejectError } = await supabase.from("orders").update({ status: "rejected" }).eq("id", orderId);if (rejectError) throw rejectError;await fetchOrders();setToast("تم رفض الطلب");} catch (e: any) {setError(e.message);} finally {setActionId(null);}};

const addToCart = (product: Product) => setCart(prev => prev.find(c => c.product.id === product.id) ? prev : [...prev, { product, quantity: 1 }]);const updateQty = (productId: number, qty: number) => setCart(prev => prev.map(c => c.product.id === productId ? { ...c, quantity: Math.max(1, qty) } : c));const removeFromCart = (productId: number) => setCart(prev => prev.filter(c => c.product.id !== productId));const cartTotal = cart.reduce((s, c) => s + c.product.price * c.quantity, 0);

const handleSubmit = async () => {if (!supplierShopId || cart.length === 0) return setModalError("بيانات ناقصة");setSaving(true);try {const { data: orderData, error: oErr } = await supabase.from("orders").insert({ from_shop_id: ownedShopId, to_shop_id: supplierShopId, status: "pending", total_amount: cartTotal, notes: orderNotes || null }).select().single();if (oErr) throw oErr;await supabase.from("order_items").insert(cart.map(c => ({ order_id: orderData.id, product_id: c.product.id, quantity: c.quantity, price: c.product.price })));setShowModal(false);setCart([]);setToast("تم إرسال الطلب بنجاح ✓");await fetchOrders();} catch (e: any) {setModalError(e.message);} finally {setSaving(false);}};
