...
export default function OrdersPage() {
  ...
  // ── Order detail drawer: open/close, item fetch, print, PDF ──
  const {
    detailOrder,
    setDetailOrder,
    detailItems,
  ...
  return (
    <div
      className="p-4 lg:p-8 min-h-screen pb-24 md:pb-10"
      dir={isRTL ? "rtl" : "ltr"}
    >
      {toast && (
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
        <div>
          ...
          <p className="text-slate-500 text-sm mt-1">
            {counts.all} {t("total", "طلب إجمالي")} ·{" "}
            {counts.pending} {t("pending", "معلق")}
            {counts.partial > 0 &&
              ` · ${counts.partial} ${t("partial", "جزئي")}`}
          </p>
        </div>
        ...
        <div className="flex items-center gap-2">
          ...
          {!isAdmin && (
            <button
              onClick={() => createOrder.setShowModal(true)}
      ...
      {error && (
        <div
          className="mt-4 p-4 rounded-xl bg-red-500/10 border border-red-500/20 text-red-400 text-sm flex items-center gap-3"
          role="alert"
        >
      ...
      {detailOrder && (
        <OrderDetailsDrawer
          ...
          onConfirmPartialApprove={() =>
            handlePartialApprove(detailOrder.id)
          }
          onApproveAll={() => handleApprove(detailOrder.id)}
          onApproveRemaining={() =>
            handleApproveRemaining(detailOrder.id)
          }
          onRejectRemaining={() =>
            handleRejectRemaining(detailOrder.id)
          }
          onCreateMissingOrder={() =>
            handleCreateMissingOrder(detailOrder)
          }
          isRTL={isRTL}
          t={t}
        />
      ...
      {createOrder.showModal && (
        <CreateOrderModal
          ...
          onSupplierChange={(id) => {
            createOrder.setSupplierShopId(id);
          }}
          ...
          visibleSupplierProductsCount={
            createOrder.visibleSupplierProducts.length
          }
